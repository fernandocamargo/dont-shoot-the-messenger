...
export default () => {
  ...
  const fetch = useCallback(() => {
    const persist = ([details, questions, difficulties]) =>
      setState(reducers.fetch({ details, difficulties, questions }));
    const promise = watch(
      Promise.all([
    ...
    return promise;
  }, [params.interview, getDifficulties, getInterview, getQuestions, watch]);
  ...
  const interview = useMemo(
    () =>
      update(state, {
        ...
        questions: {
          items: {
            $apply: (questions) => questions.reduce(reconcile, []),
          },
        },
      }),
